...
const EditBlogCard = ({ blog, blogId, handleDelete }) => {
  ...
  return (
    <div className="flex flex-col rounded glass_morphism hover:scale-95 hover:cursor-pointer transition-all duration-300 delay-300 hover:backdrop-blur-lg gap-y-4">
      <img
        src={blog?.imgUrl}
        className="max-w-full max-h-[13rem] rounded"
        alt="blog_img"
      />
      <div
        ...
        dangerouslySetInnerHTML={{
          __html:
            blog.mainContent.length > 90
              ? `${blog?.mainContent.substring(0, 80)} ...`
              : blog?.mainContent,
        }}
        className="font-sans text-white  px-2 leading-6 h-[20%]"
      >
      ...
      <div className="flex items-center pl-4 py-4">
        <button
          onClick={handleEdit}
          className="px-4 py-2 rounded text-white bg-major-text-style hover:bg-major-text-style-hover transition-all duration-300 ease-in-out"
        >
          Edit
        </button>
        <button
