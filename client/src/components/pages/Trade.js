...
import {
  ...
  disconnectExchanger,
  handleApiKeyChanges,
  startBot as controlBot,
  stopBot as handleStopBot,
...
function Trade() {
  ...
  const fetchTopPairsFromBackend = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch top pairs. Status: ${response.status}`,
        );
      }
  ...
  const fetchOpenOrders = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          `Failed to fetch open orders:${response.status} ${response.statusText}`,
        );
  ...
  const fetchOpenPositions = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          `Failed to fetch open positions: ${response.status} ${response.statusText}`,
        );
  ...
  const connectExchange = async () => {
    ...
    try {
      const response = await fetch(
        process.env.NODE_ENV == "production"
          ? "https://freedombot.online/api/set-api-keys"
          : "http://localhost:8001/api/set-api-keys",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchAutomaticTradingPairs = async () => {
    try {
      ...
      console.log(
        "Most Volatile Pair (Highest Price Change %):",
  ...
  const executeTrade = async (customTradeDecision = null) => {
    ...
    try {
      ...
      const response = await fetch(
          ...
          : "http://localhost:8001/api/execute-trade",
        {
          ...
          body: JSON.stringify({
            apiKey: !api_Key.toString().trim() ? apiKey : api_Key,
            symbol,
            apiSecretKey: !apiSecKey?.toString().trim()
              ? apiSecretKey
              : apiSecKey,
            tradeDecision: isManuallyTriggered
              ? customTradeDecision
              : tradeDecision,
            exchangeType,
            orderType,
      ...
      console.error(
        `Error executing ${tradeDecision.toLowerCase()} trade:`,
        error,
      );
    } finally {
  ...
  useEffect(() => {
    const fetchAccountInfo = async () => {
      try {
        const response = await fetch(
          process.env.NODE_ENV == "production"
            ? "https://freedombot.online/api/usdt-balance"
            : "http://localhost:8001/api/usdt-balance",
          {
            headers: {
              "X-API-KEY": api_Key,
              "X-API-SECRET-KEY": apiSecKey,
    ...
    const fetchTradeDecision = async () => {
      try {
        const response = await fetch(
          process.env.NODE_ENV == "production"
            ? `https://freedombot.online/api/trade-decision?symbol=${selectedPair}`
            : `http://localhost:8001/api/trade-decision?symbol=${selectedPair}`,
        );
        if (!response.ok) {
    ...
    const fetchHistoricalData = async () => {
      try {
        const response = await fetch(
          process.env.NODE_ENV == "production"
            ? `https://freedombot.online/api/historical-data/${selectedPair}`
            : `http://localhost:8001/api/historical-data/${selectedPair}`,
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "Trade execution is starting  with trade decision and symbol:",
        tradeDecision,
        symbol,
      );
      executeTrade();
  ...
  return (
    <>
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
        <div className="glass_morphism p-4">
          ...
          <div className="flex flex-col gap-y-3">
            <div className="flex flex-col gap-y-1">
              ...
              <input
                type="text"
                value={!api_Key.toString().trim() ? apiKey : api_Key}
                onChange={(e) => {
            ...
            <div className="flex flex-col gap-y-1">
              ...
              <input
                type="password"
                value={!apiSecKey?.toString().trim() ? apiSecretKey : apiSecKey}
                onChange={(e) => {
            ...
            <div className="flex flex-col gap-y-1">
              ...
              <select
                className="text-white border border-white p-1 px-4 outline-none bg-transparent rounded-md placeholder:text-black text-sm"
                value={exchangeType}
                onChange={(e) => setExchangeType(e.target.value)}
              >
                <option className="bg-transparent text-black" value="spot">
            ...
            <div className={`${apiSecKey && api_Key && "flex gap-x-4"}`}>
              {apiSecKey && api_Key && (
                <button
                  className="bg-white/20 text-red-500 w-[95%] md:w-[50%] mx-auto  rounded-xl p-2"
              ...
              {!isConnected ? (
                <button
                  ...
                  className="bg-white/20 text-green-500 w-[95%] md:w-[50%] mx-auto  rounded-xl p-2"
                >
                  Connect Exchange
                </button>
              ) : (
                <>
                  <button
                    ...
                    className="bg-white/20 text-red-500 w-[95%] md:w-[50%] rounded-xl p-2"
                  >
                    Disconnect Exchange
                  </button>
                </>
            ...
            <div className="flex flex-col gap-y-1">
              ...
              <div className="w-full flex flex-col md:flex-row justify-between gap-x-8 gap-y-2">
                {!isBotRunning ? (
                  <button
                    className="bg-white/20 text-green-500 w-[95%] md:w-[50%] rounded-xl p-2"
                    onClick={startBot}
                  >
                    Start Bot
                  </button>
                  ...
                  <>
                    <button
                      onClick={stopBot}
                      className="bg-white/20 text-red-500 w-[99.5%] md:w-[50%] rounded-xl p-2"
                    >
                      Stop Bot
                    </button>
                  </>
                ...
                <button
                  className={`bg-white/20 ${showAccountInfo ? "text-red-500" : "text-green-500"} w-[99.5%] md:w-[50%] rounded-xl p-2`}
            ...
            {accId && (
              <p className="text-green-500">
                Connected to Binance (Account ID: {accId})
              </p>
            )}
        ...
        <div className="glass_morphism p-4 flex flex-col gap-y-3">
          ...
          <div className="flex items-center gap-x-8 justify-center">
            <button
              className={` w-[85%] md:w-[40%] rounded-xl p-2 py-1 ${tradingManual == "automatic" ? "bg-green-500 text-white" : "bg-white/20 text-green-500"}`}
              onClick={handleAutomaticTrade}
            >
              Automatic
            </button>
            <button
              className={` w-[85%] md:w-[40%] rounded-xl p-2 py-1 ${tradingManual == "manual" ? "bg-green-500 text-white" : "bg-white/20 text-green-500"}`}
              onClick={handleManualTrade}
            >
              Manual
            </button>
            <button
              className={` w-[85%] md:w-[40%] rounded-xl p-2 py-1 ${tradingManual == "smart" ? "bg-green-500 text-white" : "bg-white/20 text-green-500"}`}
          ...
          <div className="flex flex-col gap-y-3">
            <div className="flex flex-col gap-y-1">
              ...
              {tradingManual === "smart" && (
                <div className="bg-white/10 text-white text-sm mt-2 p-2 rounded-lg flex flex-col gap-y-1">
                  <p>
                    <span className="font-bold">Smart Signal:</span>{" "}
                    {state.strategySignal || "N/A"}
                  </p>
                  <p>
                    <span className="font-bold">Strategy Score:</span>{" "}
                    {state.strategyScore ?? "N/A"}
                  </p>
                  <p>
                    <span className="font-bold">Smart Message:</span>{" "}
                    {state.bestPairMessage || "N/A"}
                  </p>
                  <p>
                    <span className="font-bold">Top Volatile Pairs:</span>{" "}
                    {state.topVolatilePairs?.join(", ") || "N/A"}
                  </p>
                  ) : (
                  <p className="text-yellow-300 italic">
                    Waiting for smart strategy recommendation...
                  </p>
                </div>
              )}
            </div>
            ...
            {showAccountInfo && accountInfo && (
              <div className="flex flex-col text-white">
                ...
                <p>
                  Asset:{" "}
                  <span className="text-green-500">{accountInfo.asset}</span>
                </p>
                <p>
                  Wallet Balance:
                ...
                <p>
                  ...
                  <span className="text-green-500">
                    {accountInfo.positionInitialMargin}
                  </span>
                </p>
                <p>
                  Open Order Initial Margin:{" "}
                  <span className="text-green-500">
                    {accountInfo.openOrderInitialMargin}
                  </span>
                </p>
              </div>
            )}
      ...
      <div className="glass_morphism p-6 flex flex-col gap-y-4 mt-6 rounded-2xl shadow-lg">
        <p className="text-white text-center text-2xl font-bold mb-2 tracking-wide">
          Open Orders
        </p>
            ...
            <div className="flex flex-col gap-2">
              {openPositions.length > 0 ? (
                openPositions.map((position) => (
                  <div
                    key={position.symbol}
                    className="bg-white/20 rounded-lg p-3 flex flex-col md:flex-row md:items-center md:justify-between"
                  >
                    <div>
                      ...
                      <span className="mx-2">|</span>
                      <span>
                        PnL: {parseInt(position.unRealizedProfit).toFixed(2)}
                      </span>
                    </div>
                    <button
