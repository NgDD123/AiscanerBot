      ...
      auth.currentUser.getIdToken().then(async (token) => {
        const response = await fetch(
          process.env.NODE_ENV === "production"
            ? `https://freedombot.online/payment/create-checkout`
            : "http://localhost:8001/payment/create-checkout",
          {
            method: "POST",
            body: JSON.stringify({
              packageType,
  ...
  return (
    <div className="h-full flex justify-center items-center">
      <div className="glass_morphism flex flex-col md:justify-center w-[98%] md:w-[50%] h-[95%] md:h-[90%] my-auto mx-auto gap-y-2 md:gap-y-4 p-3 md:p-4 ">
        <h1 className="text-center font-semibold text-white py-2 md:py-3 text-xl">
          Checkout Page
        </h1>
        ...
        <div className="flex flex-col px-2 md:px-8 gap-y-2">
          ...
          <select
            className="text-white p-2 outline-none glass_morphism"
            value={packageType}
        ...
        <div className="flex flex-col px-2 md:px-8 gap-y-2">
          ...
          <select
            className="text-white p-2 outline-none glass_morphism bg-transparent"
            value={coinType}
            onChange={(e) => setCoinType(e.target.value)}
          >
            <option className="text-black" value="USDT.BEP20">
              USDT.BEP20
            </option>
        ...
        <button
          className="bg-white rounded-xl text-major-text-style w-40 mx-auto font-semibold p-3"
          onClick={getPaymentUrl}
          disabled={isLoading}
        >
          {isLoading ? "Loading..." : "Process Payment"}
        </button>
      </div>
    </div>
  );
